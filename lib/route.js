...
var route = function ( method, url, actions, action, args, next ){
  ...
  var _action = actions[ file ][ fn ]( args, function (){
  ...
  request( opt, function (  err, res, body ){
    var tmp     = [ '[ ', file, ' ]', '[ ', fn, ' ]' ].join( '' );
    var $action = colors.bgBlue.white.bold( tmp );
    var $others = colors.yellow( method.toUpperCase() + ' : ' + opt.uri );
    var info    = $action + ' ' + $others;

    console.log( info );
    handler( err, res, body, inline_log );
  });
};
